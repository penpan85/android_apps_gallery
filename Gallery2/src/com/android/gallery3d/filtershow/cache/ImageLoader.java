...
public final class ImageLoader {
     ...
     * null.
     * 如果传入的参数矩形是uri对应图片的区域矩形,那么返回此矩形所限定的bitmap，否则返回null
     */
    public static Bitmap loadRegionBitmap(Context context, BitmapCache cache,
        ...
        try {
            ...
            //如果图片流数据并没有包含需要的rect，取
            if (!r.contains(imageBounds)) {
            	//r中有与imageBounds相交的部分，则将imageBounds置为两者的交集，否则imageBounds维持原状
     ...
     * orientation.
     * 解码给定uri的图片，对其的samplesize进行重新计算，使其宽高不超过给定最大维度
     *
     * @param uri URI of image to open.
     * @param context context whose ContentResolver to use.
     * @param maxSideLength max side length of returned bitmap.
     ...
     * 尝试以原始尺寸解码一张图片，如果在低配置设备上发生了溢出异常，增大samplesize
     */
     ...
     * 至少以给点的缩放率解码一张图片，如果发生了溢出，尝试以更大的samplesize解码图片
     */
     ...
     * @param context
     * @param uri
     * @return 从媒体库中获取到所有的exif格式图片的信息
     * exif:一种数码相片的格式，其实就是jpeg加入了光圈，白平衡等等信息
     */
