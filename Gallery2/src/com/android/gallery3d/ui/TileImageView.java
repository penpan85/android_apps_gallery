...
public class TileImageView extends GLView {
     ...
     *              --> IN_QUEUE - by queueForDecode() 队列中待解码
     *              --> RECYCLED - by recycleTile() 回收
     *      IN_QUEUE --> DECODING - by decodeTile() 解码中
     *               --> RECYCLED - by recycleTile) 回收
     *      DECODING --> RECYCLING - by recycleTile() 回收中
     *               --> DECODED  - by decodeTile() 解码完成
     *               --> DECODE_FAIL - by decodeTile() 解码失败
    ...
    // 变量 mlevel用来指示哪一级的bitmap应当被使用，0代表原始尺寸的bitmap,更大的数字代表被缩放了一定倍率的bitmap
    // ,缩放的倍率变动区间:[0,mLevelCount),我们使用mScaledBitmaps[mLevel]来显示图片，如果使用倍率达到最大
    // 值mLevelCount,这意味着我们使用mScreenNail来显示图片
    private int mLevel = 0;

    // The offsets of the (left, top) of the upper-left tile to the (left, top)
    // of the view.
    // 顶部左边的tile到view的顶部左边的偏移量
    ...
    private void layoutTiles(int centerX, int centerY, float scale, int rotation) {
        ...
        // 我们需要保持两级以上的材质，这样可以提高缩放率变化到下个等级时的速度，
    ...
    // 如果bitmap在给定的因子scale下被缩放了，返回包含可视范围内的rectangle,左顶点坐标与瓦片边界对齐
    // (cX, cY)即缩放的中心点
    // 
